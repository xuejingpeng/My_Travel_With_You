...
- (IBAction)save:(UIBarButtonItem *)sender {
    if ([self.isViewContrller isEqualToString:@"住宿"]) {
        ...
        BmobObject *obj = [BmobObject objectWithoutDataWithClassName:@"accommodation" objectId:self.accommodation.objectId];
...
//                self.objectId = [obj objectId];
        ...
        BmobObject *bmobObject = [BmobObject objectWithoutDataWithClassName:@"AccommodationCollection" objectId:self.objectId];
        ...
        if(i%2 == 0){
            ...
            self.navigationItem.rightBarButtonItem.tintColor = [UIColor whiteColor];
            BmobObject *bmobObject = [BmobObject objectWithoutDataWithClassName:@"FoodCollection" objectId:self.objectId];
            [bmobObject deleteInBackgroundWithBlock:^(BOOL isSuccessful, NSError *error) {
                if (isSuccessful) {
                    [MBProgressHUD showSuccess:@"取消收藏"];
                    i++;
        ...
        if(i%2 == 0){
            ...
            self.navigationItem.rightBarButtonItem.tintColor = [UIColor whiteColor];
            BmobObject *bmobObject = [BmobObject objectWithoutDataWithClassName:@"TravelCollection" objectId:self.objectId];
            [bmobObject deleteInBackgroundWithBlock:^(BOOL isSuccessful, NSError *error) {
                if (isSuccessful) {
                    [MBProgressHUD showSuccess:@"取消收藏"];
                    i++;
